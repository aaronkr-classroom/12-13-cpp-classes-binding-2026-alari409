...
void Vec<T>::create(size_type n, const T& val) {
	data = alloc.allocate(n); // #include <memory> <<< 수정 >>>
	limit = avail = data + n;
...
void Vec<T>::uncreate() {
	if (data) {
		...
		while (it != data) // 첫 번째 요로를 가리키는 포인터까지
			alloc.destroy(--it);
...
typename Vec<T>::iterator Vec<T>::insert(iterator d, In b, In e) {
	...
	// 다시 끝 부분 추가하기 (불러오기)
	for (typename Vec<T>::const_iterator it = end.begin();
		it != end.end(); ++it)
		push_back(*it);
